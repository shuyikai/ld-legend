...
namespace ET.Server
{
    public class AI_Follow2 : AAIHandler
    {
        ...
        public override async ETTask Execute(AIComponent aiComponent, AIConfig aiConfig, ETCancellationToken cancellationToken)
        {
            ...
            */
            long oldSpeed = unit.GetComponent<NumericComponentServer>().GetAsLong(NumericType.Base_Speed_Base);
            
            long masterspeed = master.GetComponent<NumericComponentServer>().GetAsLong(NumericType.Now_Speed);
            unit.GetComponent<NumericComponentServer>().ApplyValue(NumericType.Base_Speed_Base, masterspeed);
